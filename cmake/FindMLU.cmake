...
# Check CNNLExtra version.
if (EXISTS "${NEUWARE_INCLUDE_DIR}/cnnl_extra.h")
  set(_file ${NEUWARE_INCLUDE_DIR}/cnnl_extra.h)
  file(READ ${_file} tmp)
  string(REGEX MATCH "define CNNL_EXTRA_MAJOR * +([0-9]+)" _major "${tmp}")
  string(REGEX REPLACE "define CNNL_EXTRA_MAJOR * +([0-9]+)" "\\1" _major "${_major}")
  string(REGEX MATCH "define CNNL_EXTRA_MINOR * +([0-9]+)" _minor "${tmp}")
  string(REGEX REPLACE "define CNNL_EXTRA_MINOR * +([0-9]+)" "\\1" _minor "${_minor}")
  string(REGEX MATCH "define CNNL_EXTRA_PATCHLEVEL * +([0-9]+)" _patch "${tmp}")
  string(REGEX REPLACE "define CNNL_EXTRA_PATCHLEVEL * +([0-9]+)" "\\1" _patch "${_patch}")  
  set(CNNL_EXTRA_VERSION ${_major}.${_minor}.${_patch})
  set(CNNL_EXTRA_VERSION_MAJOR ${_major})
  set(CNNL_EXTRA_VERSION_MINOR ${_minor})
  set(CNNL_EXTRA_VERSION_PATCH ${_patch})
  list(APPEND MLU_LIBRARIES cnnl_extra)
  get_filename_component(_dir "${NEUWARE_ROOT_DIR}/include" ABSOLUTE)
  message(STATUS "Found CNNLExtra: ${_dir} (found version \"${CNNL_EXTRA_VERSION}\")")
else()
  message(FATAL_ERROR "CNNLExtra is not found.")
endif()
